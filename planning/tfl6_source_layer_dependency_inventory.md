...
| `tfl6_nd_020` | `existing_roads` | Candidate public road sources; likely BC road atlas / forest road data, plus MP10 width rules | Existing-road line/polygon overlay and buffer exclusion | Missing source | Run FEMIC/BCDC source resolution for road datasets; separate existing roads from future-road allowance. |
...
| `tfl6_nd_060` | `operability` | Historical WFP/TFL operability source if available; otherwise reviewed proxy | Operability class exclusion for `I`, `Ocm`, `Ohm` semantics | Missing source / proxy decision | Resolve whether public/local operability geometry exists; if not, record proxy or benchmark fallback. |
...
1. Roads: existing-road geometry and future-road treatment split.
2. Operability: public/local source availability or reviewed proxy.
3. Hydrology/wetland/shoreline: riparian source bundle and rule contract.
4. Legal overlays: UWR, WHA, and established OGMAs with current-vs-2011
   vintage warnings.
5. Recreation features: sites/trails and 10 m buffer source.
6. Strata attribution: LU, RMZ, and BEC source/schema for stand-level retention.
7. Historical/fallback rows: draft OGMAs and cultural heritage proxy/aspatial
   handling.
